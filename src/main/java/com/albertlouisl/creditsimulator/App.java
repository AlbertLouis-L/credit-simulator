...
public class App {
    ...
    private static void creditHandler(Scanner scanner){
        ...
        ArrayList<MonthlyInstallment> result =
                calculateCredit(jenisKendaraan, statusKendaraan, tahunKendaraan,
                        jumlahPinjaman, tenor, jumlahDP);

        // System.out.println("=== Hasil Simulasi Kredit ===");
        // for (MonthlyInstallment m : result) {
        //     System.out.printf(
        //             "Tahun %d | Bunga %.2f%% | Cicilan Bulanan: %.2f%n",
        //             m.tahun, m.bunga, m.monthlyInstallment
        //     );
        System.out.println("=== Hasil Simulasi Kredit ===");
        ...
        for (MonthlyInstallment m : result) {
            String cicilan = rupiah.format(m.monthlyInstallment);
            String bungaFormatted = formatBunga(m.bunga);
